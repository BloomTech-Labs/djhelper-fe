...
test('About page renders correctly', () => {
    ...
    const step1aText = queryByText('Create a DJ account and make pages for your events.');
    const step1bText = queryByText('Share the event pages with your clients.');

    const step2aText = queryByText('Clients create song requests on the events pages.');
    const step2bText = queryByText('Clients also upvote the requests they want to hear.');
    
    const step3aText = queryByText('See which songs receive the most votes.');
    const step3bText = queryByText('Get additional recommendations from the app, based on those song requests.')
